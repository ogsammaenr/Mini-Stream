...
# Varsayılan Hedef
all: baslangic ministream.so generator test_temel test_benchmark bitis
...
# Hedef : Performans Benchmark Testleri
...
# Hedef: Sadece Bellek Sızıntısı (Memory Leak) Testi
test_bellek: $(CORE_SRCS) test/test_bellek.c
	@echo -e "$(CYAN)🧪 Bellek Sızıntısı Testi derleniyor...$(RESET)"
	@$(CC) $(CFLAGS) -I src test/test_bellek.c $(CORE_SRCS) -o test_bellek
	@echo -e "$(GREEN)   ✔ Başarılı!$(RESET)"

# Hedef : Multithread Ağır Yük Testi
test_multithread: $(CORE_SRCS) test/test_multithread.c
	@echo -e "$(CYAN)🔥 Multithread Ağır Yük Testi derleniyor...$(RESET)"
	@$(CC) $(CFLAGS) -I src test/test_multithread.c $(CORE_SRCS) -o test_multithread
	@echo -e "$(GREEN)   ✔ Başarılı!$(RESET)"

multithread: test_multithread
	@./test_multithread

test: test_temel test_benchmark test_bellek
	@./test_temel
	@./test_benchmark
	@./test_bellek	
...
bitis:
	...
	@echo -e "$(YELLOW)👉 temel testler icin		:$(RESET) make test"
	@echo -e "$(YELLOW)👉 multithread testleri icin	:$(RESET) make multithread"
	@echo -e "$(YELLOW)👉 Veri üretmek için   		:$(RESET) ./generator [Üretmek istediğiniz veri miktarı]"
